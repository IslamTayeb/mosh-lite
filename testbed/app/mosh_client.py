...
def get_server_ips():
    """Get all IP addresses for the server on different networks

    Discovers server IPs by checking our own network interfaces and
    deriving the server IP (server is always .2, client is .3 in our setup)
    """
    import subprocess
    ...
    try:
        # Get our own IP addresses to understand which networks we're on
        ...
        # Parse the output to find our IPs and derive server IPs
        for line in result.stdout.split('\n'):
            if 'inet ' in line and '127.0.0.1' not in line:
                parts = line.strip().split()
                if len(parts) >= 2:
                    my_ip = parts[1].split('/')[0]
                    # Derive server IP (server is .2, we are .3)
                    if my_ip.startswith('10.1.') and my_ip.endswith('.3'):
                        network_ips['wifi'] = my_ip.rsplit('.', 1)[0] + '.2'
                    elif my_ip.startswith('10.2.') and my_ip.endswith('.3'):
                        network_ips['cellular_1'] = my_ip.rsplit('.', 1)[0] + '.2'
                    elif my_ip.startswith('10.3.') and my_ip.endswith('.3'):
                        network_ips['cellular_2'] = my_ip.rsplit('.', 1)[0] + '.2'

        if network_ips:
            log(f"Discovered server IPs (derived from client IPs):")
            for network, ip in sorted(network_ips.items()):
                log(f"  {network}: {ip}")
            return network_ips
        else:
            log("WARNING: Could not discover server IPs, will use hostname")
            return None

    except Exception as e:
        log(f"WARNING: Could not get server IPs: {e}, will use hostname")
        return None


def wait_for_server(sock, server_address, timeout=30):
    """Wait for server to be reachable"""
    log(f"Waiting for server {server_address[0]}:{server_address[1]} to be reachable...")
    start = time.time()
    ...
    while time.time() - start < timeout:
        try:
            ...
            test_packet = json.dumps(
                {"seq": -1, "timestamp": time.time(), "test": True}
            ).encode("utf-8")
            ...
            log(f"Server {server_address[0]}:{server_address[1]} is reachable!")
            return True
...
def run_phase(
    sock, phase_num, phase_config, sent_packets, received_packets, seq_counter, server_address
):
    ...
    active_interface = phase_config.get("active_interface", None)

    log(f"\n{'='*60}")
    log(f"Phase {phase_num}: {phase_config.get('description', 'No description')}")
    log(f"  Duration: {phase_duration}s")
    log(
        f"  Network: delay={phase_config.get('delay_ms', 0)}ms, "
        f"jitter={phase_config.get('jitter_ms', 0)}ms, "
        f"loss={phase_config.get('loss_pct', 0)}%, "
        f"rate={phase_config.get('rate_kbit', 0)}kbit"
    )
    if active_interface:
        log(f"  Active interface: {active_interface}")
    log(
        f"  Sending packets continuously for {phase_duration}s at ~{int(1/SEND_RATE)} pps..."
    )
    log(f"  Target: {server_address[0]}:{server_address[1]}")
    log(f"{'='*60}")
    ...
    while time.time() < phase_end and not should_exit:
        send_time = time.time()
        seq = seq_counter[0]
        packet = {"seq": seq, "timestamp": send_time, "phase": phase_num}
        ...
        try:
            ...
            if phase_sent_count % 100 == 0 and phase_sent_count > 0:
                ...
                log(
                    f"  Sent {phase_sent_count} packets ({elapsed:.1f}s / {phase_duration}s)"
                )
    ...
    log(
        f"  Phase transmission complete: {phase_sent_count} packets sent in {send_duration:.2f}s"
    )

    # Calculate phase statistics (only for packets sent in this phase)
    phase_received = {
        s: received_packets[s]
        for s in range(phase_start_seq, seq_counter[0])
        if s in received_packets
    }
    ...
    log(
        f"    Packets: {packets_sent} sent, {packets_received} received, {packets_lost} lost ({loss_percent:.2f}%)"
    )
    if results["latency_ms"]:
        lat = results["latency_ms"]
        log(
            f"    Latency: min={lat['min']}ms, max={lat['max']}ms, "
...
def save_results(results, scenario_info):
    ...
    try:
        with open(SUMMARY_FILE, "w") as f:
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"Total Packets Sent:     {output['overall']['total_packets_sent']}\n"
            )
            f.write(
                f"Total Packets Received: {output['overall']['total_packets_received']}\n"
            )
            f.write(
                f"Total Packets Lost:     {output['overall']['total_packets_lost']}\n"
            )
            f.write(
                f"Overall Loss:           {output['overall']['overall_loss_percent']:.2f}%\n"
            )
            if output["overall"]["mean_latency_ms"]:
                f.write(
                    f"Mean Latency:           {output['overall']['mean_latency_ms']:.3f}ms\n"
                )
            f.write("\n")
            ...
            for r in results:
                f.write(
                    f"Phase {r['phase']}: {r['config'].get('description', 'No description')}\n"
                )
                f.write("-" * 80 + "\n")
                ...
                f.write(
                    f"  Packets Lost:     {r['packets_lost']} ({r['loss_percent']:.2f}%)\n"
                )
...
def main():
    ...
    default_server_address = (SERVER_HOST, UDP_PORT)

    # Create UDP socket
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

    # Wait for server (try first available IP if we have them, otherwise use hostname)
    if server_ips:
        # Try to reach server on any of its IPs
        reachable = False
        for network, ip in server_ips.items():
            if wait_for_server(sock, (ip, UDP_PORT), timeout=10):
                reachable = True
                break
        if not reachable:
            log("ERROR: Server not reachable on any network")
            sys.exit(1)
    else:
        if not wait_for_server(sock, default_server_address):
            ...
            try:
                ...
                if resp_seq in sent_packets and resp_seq not in received_packets:
                    ...
                    if len(received_packets) % 100 == 0:
                        log(
                            f"  [Receiver] {len(received_packets)} responses received total"
                        )
    ...
    for i, step in enumerate(steps):
        ...
        # Determine which server address to use for this phase
        active_interface = step.get("active_interface", None)
        if active_interface and server_ips and active_interface in server_ips:
            # Use the specific network's IP
            server_address = (server_ips[active_interface], UDP_PORT)
        else:
            # Use default (hostname resolution)
            server_address = default_server_address

        result = run_phase(
            sock, phase_num, step, sent_packets, received_packets, seq_counter, server_address
        )
    ...
    log(f"\n{'='*60}")
    log(
        f"All phases complete. Waiting {RESPONSE_WAIT_TIME}s for remaining responses..."
    )
    ...
    log(
        f"Receiver stopped. Total responses: {len(received_packets)}/{len(sent_packets)}"
    )
